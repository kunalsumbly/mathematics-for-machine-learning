...
def row_echelon_form(A, B):
    ...
    for row in range(num_rows):
        ...
        print(f'Pivot candidate: {pivot_candidate}')
        ...
        if np.isclose(pivot_candidate, 0) == True:
            ...
            print(f'First non-zero value below pivot candidate: {first_non_zero_value_below_pivot_candidate}')
            # Swap rows
            print(f'Swapping rows {row} and {first_non_zero_value_below_pivot_candidate}')
        ...
        for j in range(row + 1, num_rows):
            ...
            print(f'Row below pivot value of j: {j}')
            print(f'row: {row}')
            value_below_pivot = M[j, row]
            print(f'Value below pivot: {value_below_pivot}')
...
def back_substitution(M):
    ...
    for row in reversed(range(num_rows)):
        ...
        print(f'Index of first non-zero element in row {substitution_row}: {index}')
        ...
        for j in reversed(range(row)):
            ...
            row_to_reduce = M[j]
            print(f'Row to reduce : {row_to_reduce} and j : {j}')
            # Get the value of the element at the found index in the row to reduce
            value = row_to_reduce[row]
            print(f'Value of element at index {index} in row {row_to_reduce}: {value}')
            ...
            print(f'Updated row to reduce: {row_to_reduce}')
